...
ratings = pd.read_csv('D:\\sr-wsdream\\dataset\\sample\\ratings.csv')
...
for u in users.UserID:
    ratings_new[(ratings_new['UserID'] == u) & (ratings_new['Timestamp'] < ratings_new[ratings_new['UserID'] == u]['Timestamp'].quantile(0.7))] = np.nan
    ratings_old[(ratings_old['UserID'] == u) & (ratings_old['Timestamp'] > ratings_old[ratings_old['UserID'] == u]['Timestamp'].quantile(0.7))] = np.nan
ratings_new.dropna(inplace=True)
ratings_old.dropna(inplace=True)
s_rate_new = s_rate.copy()
s_rate_new[(rate_time < rate_time.quantile(0.7))] = np.nan
s_rate_old = s_rate.copy()
s_rate_old[(rate_time > rate_time.quantile(0.7))] = np.nan
...
ratings_new.to_csv('D:\\sr-kl\\dataset\\测试\\ratings_ceshiji.csv')
ratings_old.to_csv('D:\\sr-kl\\dataset\\测试\\ratings_xunlianji.csv')
s_rate.to_csv('D:\\sr-kl\\dataset\\测试\\s_rate.csv')
s_rate_new.to_csv('D:\\sr-kl\\dataset\\测试\\s_rate_ceshiji.csv')
s_rate_old.to_csv('D:\\sr-kl\\dataset\\测试\\s_rate_xunlianji.csv')
rate_time_new.to_csv('D:\\sr-kl\\dataset\\测试\\rate_time_ceshiji.csv')
rate_time_old.to_csv('D:\\sr-kl\\dataset\\测试\\rate_time_xunlianji.csv')
rate_time.to_csv('D:\\sr-kl\\dataset\\测试\\rate_time.csv')
